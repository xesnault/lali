...
QList<List> ListRepository::getAll() {
...
void ListRepository::save(QList<List> lists) {
	cachedLists = lists;
...
QList<List> ListRepository::loadLists() {
	...
	for (auto listJson : listsJson) {
		List list(listJson.toObject()["name"].toString());
